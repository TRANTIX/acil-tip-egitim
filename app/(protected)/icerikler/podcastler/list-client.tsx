...
export function PodcastListClient({ podcasts, categories, formats }: Props) {
  const [search, setSearch] = useState("");
  const [activeCategory, setActiveCategory] = useState("hepsi");
  const [activeFormat, setActiveFormat] = useState("hepsi");
  ...
  const filtered = useMemo(() => {
    return podcasts.filter((p) => {
      ...
      const matchCategory =
        activeCategory === "hepsi" || p.category === activeCategory;
      const matchFormat =
        activeFormat === "hepsi" || p.format === activeFormat;
      return matchSearch && matchCategory && matchFormat;
    });
  }, [podcasts, search, activeCategory, activeFormat]);
  ...
  return (
    <div>
      ...
      <div className="relative mb-6">
        ...
        <input
          type="text"
          placeholder="Podcast ara (başlık, açıklama, etiket...)"
          value={search}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button
          onClick={() => setActiveFormat("hepsi")}
          className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
            activeFormat === "hepsi"
              ? "bg-purple-600 text-white"
              : "border border-[var(--border)] text-[var(--muted-foreground)] hover:border-purple-600/50 hover:text-[var(--foreground)]"
          }`}
        >
          Tümü ({podcasts.length})
        </button>
        {formats.map((f) => {
          const count = podcasts.filter((p) => p.format === f.value).length;
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        ...
        {categories.map((cat) => {
          const count = podcasts.filter((p) => p.category === cat).length;
          if (count === 0) return null;
          return (
            <button
              key={cat}
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-10 text-center text-[var(--muted-foreground)]">
          <Headphones className="mx-auto h-10 w-10 mb-3 opacity-40" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((podcast) => (
            <Link key={podcast.id} href={`/icerikler/podcastler/${podcast.id}`}>
              <div className="group flex flex-col h-full rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 hover:border-purple-700/60 hover:shadow-md transition-all cursor-pointer">
                {/* Üst bölüm */}
                <div className="flex items-start justify-between mb-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-950/40 border border-purple-900/40 text-lg">
                    {FORMAT_ICONS[podcast.format || ""] || <Headphones className="h-5 w-5 text-purple-400" />}
                  </div>
                  <div className="flex items-center gap-2">
                    {podcast.difficulty && (
                      <span className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${DIFFICULTY_COLORS[podcast.difficulty] || ""}`}>
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-[var(--foreground)] text-sm leading-tight mb-2">
                    {podcast.episode_number && (
                      <span className="text-purple-400 mr-1">#{podcast.episode_number}</span>
                    )}
                    {podcast.title}
                  </h3>
